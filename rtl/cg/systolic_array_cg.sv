// Gated-PE NxN systolic array: identical structure/wiring to systolic_array.sv, plus an
// act_valid_in per row that rides alongside act_in through the pe_cg act_valid_out chain.
module systolic_array_cg #(parameter N = 4) (
    input  logic        clk,
    input  logic        rst,
    input  logic        weight_load,
    input  logic signed [7:0]  weight_in    [0:N-1][0:N-1],
    input  logic signed [7:0]  act_in       [0:N-1],
    input  logic               act_valid_in [0:N-1],
    output logic signed [31:0] psum_out     [0:N-1]
);
    logic signed [7:0]  act_wire       [0:N-1][0:N];
    logic               act_valid_wire [0:N-1][0:N];
    logic signed [31:0] psum_wire      [0:N][0:N-1];

    genvar r, c;

    generate
        for (r = 0; r < N; r++) begin : rows
            for (c = 0; c < N; c++) begin : cols
                logic signed [7:0]  a_in;
                logic               v_in;
                logic signed [31:0] p_in;
                assign a_in = (c == 0) ? act_in[r]       : act_wire[r][c];
                assign v_in = (c == 0) ? act_valid_in[r]  : act_valid_wire[r][c];
                assign p_in = (r == 0) ? 32'sd0           : psum_wire[r][c];

                pe_cg u_pe (
                    .clk(clk), .rst(rst),
                    .weight_load(weight_load), .weight_in(weight_in[r][c]),
                    .act_valid(v_in), .act_in(a_in), .psum_in(p_in),
                    .act_out(act_wire[r][c+1]), .act_valid_out(act_valid_wire[r][c+1]),
                    .psum_out(psum_wire[r+1][c])
                );
            end
        end
    endgenerate

    generate
        for (c = 0; c < N; c++) assign psum_out[c] = psum_wire[N][c];
    endgenerate
endmodule
