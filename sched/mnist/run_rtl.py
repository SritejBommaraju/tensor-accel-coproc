#!/usr/bin/env python3
"""Run the INT8 MNIST MLP end-to-end through the tiling scheduler on Verilated RTL."""
import argparse
import os
import sys
import time
...
from tiler import plan, execute, VerilatorBackend, NumpyBackend  # noqa: E402
...
def main():
    ...
    # layer 1: (n, 784) @ (784, 64) on RTL
    schedule1 = plan(M=n, K=784, N=64)
    acc1_rtl, stats1 = execute(schedule1, x_i8, W1, backend)

    # bit-exactness check vs numpy int8 matmul (pre-bias, raw accumulator from RTL == numpy int8 matmul)
    ...
    cycles = stats1["cycles"] + stats2["cycles"]
